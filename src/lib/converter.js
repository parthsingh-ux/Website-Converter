...
export function cleanMediaUrl(value) {
  let str = String(value || '').trim().replace(/^(["'])(.*)\1$/, '$2');
  const embeddedHttp = str.match(/(https?:\/\/[^\s"']+)/i) || str.match(/(https?:\/[^\s"']+)/i);
  if (embeddedHttp) {
    return embeddedHttp[1].replace(/^(https?:\/)(?!\/)/i, '$1/');
  }
  return str;
}
...
export function resolveMediaUrl(value, mediaPrefix = '') {
  const url = cleanMediaUrl(value);
  if (!url || /^#/.test(url) || /^data:/i.test(url)) return url;

  if (/^\/\//.test(url)) {
    const proto = mediaPrefix && /^http:/i.test(mediaPrefix) ? 'http:' : 'https:';
    return `${proto}${url}`;
  }

  if (/^https?:\/\//i.test(url)) return url;

  if (!mediaPrefix.trim()) return url;

  const prefix = mediaPrefix.trim();
  const filename = cleanFilename(url);
  const now = new Date();
  const year = now.getFullYear();
  const month = String(now.getMonth() + 1).padStart(2, '0');

  if (/^(https?:\/\/)?([a-z0-9.-]+\.[a-z]{2,})/i.test(prefix)) {
    try {
      const base = /^https?:\/\//i.test(prefix) ? prefix : `https://${prefix}`;
      if (url.includes('/wp-content/uploads/') || url.startsWith('/')) {
        const baseUrl = base.endsWith('/') ? base : `${base}/`;
        return new URL(url, baseUrl).href;
      }
      const cleanDomain = base.replace(/\/wp-content\/uploads\/?$/i, '').replace(/\/$/, '');
      return `${cleanDomain}/wp-content/uploads/${year}/${month}/${filename}`;
    } catch {
      // Fallback
    }
  }
...
export function rewriteCssUrls(css, mediaPrefix) {
...
export function serializeCommentAttributes(attributes) {
...
export function validateGutenbergMarkup(markup) {
  const stack = [], issues = [];
...
export async function convertHtmlToGutenbergJson({ html, css = '', js = '', title = 'Converted Page', mediaPrefix = '', imageAssets = [] }) {
  ...
  $('html,body,body *').each((_, el) => {
    ...
    if (el.name === 'img') {
      const dataAttrs = ['data-src', 'data-lazy-src', 'data-original', 'data-url', 'data-hi-res-src', 'data-src-large', 'data-large-file', 'data-full-url'];
      let originalSrc = $el.attr('src') || '';
      const isPlaceholder = !originalSrc || /^data:image\/(svg\+xml|gif);/i.test(originalSrc) || /blank\.gif|1x1|spacer|placeholder/i.test(originalSrc);

      if (isPlaceholder) {
        for (const attr of dataAttrs) {
          const val = $el.attr(attr);
          if (val && !/^data:image\/(svg\+xml|gif);/i.test(val) && !/blank\.gif|1x1|spacer|placeholder/i.test(val)) {
            originalSrc = val;
            break;
          }
        }
      ...
      let originalSrcset = $el.attr('srcset') || '';
      if (!originalSrcset || /blank\.gif|1x1|spacer|placeholder/i.test(originalSrcset)) {
        originalSrcset = $el.attr('data-srcset') || $el.attr('data-lazy-srcset') || '';
      }
      $el.removeAttr('data-srcset').removeAttr('data-lazy-srcset');

      const resolvedSrc = resolveMediaUrl(originalSrc, mediaPrefix);
      $el.attr('src', resolvedSrc);

      if (originalSrcset && !/^data:/i.test(originalSrcset)) {
        $el.attr('srcset', originalSrcset.split(',').map(part => {
  ...
  function convert(el) {
    ...
    if (MARKUP_TAGS.has(el.name) || (['i', 'span'].includes(el.name) && !$(el).text().trim() && (el.attribs.class || el.attribs['aria-hidden']))) return { name: 'gcb/markup', attributes: { content: $.html(el) } };
  ...
  return {
    ...
    patternCss, patternJs, imagesArray: imageAssets,
